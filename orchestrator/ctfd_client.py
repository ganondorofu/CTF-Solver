...
class CTFdClient:
    ...
    def get_solved_challenge_ids(self) -> set[int]:
        """
        自チーム（またはユーザー）が解答済みの問題IDセットを返す。

        チームモードでは /teams/me/solves を優先し、
        403/404 の場合は /users/me/solves にフォールバックする。
        """
        for endpoint in ("/teams/me/solves", "/users/me/solves"):
            try:
                ...
                if e.response is not None and e.response.status_code in (403, 404):
                    logger.debug("エンドポイント %s は利用不可 (%s)、次を試みます", endpoint, e)
                    continue
                raise
        logger.warning("解答済み問題の取得に失敗しました（全エンドポイントが利用不可）")
        return set()
