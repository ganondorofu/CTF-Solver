...
class BaseRunner:
    ...
    def __init__(self, agent_name: str):
        ...
        self.work_dir = self.workspace  # cwd=/workspace（chall/等にアクセス可能にする）
        self.chall_dir = self.workspace / "chall"
        self.shared_info_dir = self.workspace / "SharedInfo"

        # タイムアウト設定（環境変数から取得、0=無制限、デフォルト550秒）
        _timeout = int(os.environ.get("AGENT_TIMEOUT", "550"))
        self.timeout = None if _timeout == 0 else _timeout
    ...
    def extract_flag_from_output(self, output: str) -> str | None:
        ...
        # プロンプト内の例示フラグを除外（エコーバックによる誤検出防止）
        try:
            prompt_text = self.prompt_path.read_text(encoding="utf-8", errors="replace")
            prompt_flags = re.findall(r'[A-Za-z0-9_]+\{[^}\\\n\r"\']{3,}\}', prompt_text)
            placeholders.update(prompt_flags)
        except Exception:
            pass
        ...
        for pattern in patterns:
            ...
            for match in matches:
                ...
                if inner and len(inner.group(1)) >= 3 and inner.group(1) != "...":
                    ...
                    if self._CODE_PATTERNS.search(inner_text):
                        continue
    ...
    _CODE_PATTERNS = re.compile(
        r'(?:'
        r'if\s|else\s|for\s|while\s|import\s|from\s|def\s|class\s|return\s'
        r'|\.find|\.search|\.match|\.replace|\.split|\.join|\.format'
        r'|\\n|\\r|\\t|\\\\'
        r'|\bin\s+resp\b|\bin\s+line\b|\bin\s+output\b'
        r'|re\.\w+|rb["\']|r["\']'
        r')'
    )
    _SUCCESS_KEYWORDS = (
